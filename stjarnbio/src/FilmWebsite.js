  ...
  return (
    <div>
      ...
      <header className="navbar">
        <img src={`${process.env.PUBLIC_URL}/logo.svg`} alt="Stjärn Bio" className="logo" />
        <button className="menu-toggle" onClick={toggleMenu} aria-expanded={menuOpen} aria-controls="primary-navigation" aria-label={menuOpen ? 'Stäng meny' : 'Öppna meny'}>
          {menuOpen ? <X size={30} /> : <Menu size={30} />}
        </button>
        <nav id="primary-navigation" className={menuOpen ? 'nav-menu open' : 'nav-menu'} aria-label="Huvudmeny">
      ...
      <section className="background" role="banner">
        ...
        <button onClick={scrollToFilms} className="main-cta">Se Aktuella Filmer</button>
      </section>
      ...
      {/* Footer */}
      <footer>
        <img src={`${process.env.PUBLIC_URL}/logo.svg`} alt="Stjärn Bio" className="footer-logo" />
      ...
      {!cookiesAccepted && (
        <div className="cookie-popup" role="alert" aria-live="assertive">
          <p>
            ...
            <button className="accept-button" onClick={acceptCookies}>Acceptera</button>
          </p>
        </div>
      )}
    </div>
